...
#### Claude Code 를 실행하는 3가지 환경

**여기가 많이들 헷갈리는 부분**입니다. 같은 "Claude Code" 인데 **어디서 띄우냐**에 따라 옵션이 다릅니다.

| 환경 | 특징 | Chrome in Claude 등 Connector | 추천 대상 |
|---|---|---|---|
| **Claude Desktop 앱** 의 **"Code" 탭** (최근 내장) | 로그인하면 대화 옆에 Code 탭 바로 등장. 별도 에디터 불필요 | ✅ Connectors 붙여서 사용 가능 | **비개발자 최우선 추천** |
| **VS Code / Cursor / Antigravity** 의 Claude Code 확장 | 코드 편집기 안에서 파일 트리 보며 작업 | ❌ 직접 연결 불가 (MCP 수동 설정 필요) | 코드 편집 위주 개발자 |
| **터미널 (CLI)** `claude` 명령 | 검은 창에 텍스트만. 가장 가벼움 | ❌ 직접 연결 불가 | 서버 환경·스크립트 자동화 |

> 💡 **꿀팁 (제가 배운 교훈)**
> 저는 매뉴얼 작성 중에 "Claude Desktop 과 Claude Code 는 완전 별도 앱" 이라고 잘못 알고 있었어요. 공식 문서(https://code.claude.com/docs/en/desktop-quickstart) 확인 결과 **Claude Desktop 앱 안에 "Code" 탭이 내장**되어 있더라구요. **비개발자는 이 Desktop 방식이 가장 편합니다** — 한 앱 안에서 대화 + 코드 + Chrome Connector 전부 쓸 수 있어요.

> 🌐 **"그래서 Chrome in Claude 는?"** — Desktop 앱의 Code 탭이면 **OK**. VS Code 의 Claude Code 면 직접 연결 안 되니 `playwright-cli` 스킬이나 MCP 수동 설정을 써야 합니다 (섹션 8 Tip 1 참고).
...
### 7-1. GitHub 에 코드 올리기 — 처음 해보는 사람용 8단계 가이드

> 🎯 **이 섹션의 존재 이유**
> "git 이 뭔지도 모르는데 커밋? 푸시? 저장소?" — 저도 처음에 이 단어들 하나도 몰랐습니다. 아래는 **완전 처음** 하는 분을 위한 순서 전체.
>
> ⏱️ 15~20분 · 🟡 중간 (용어가 낯설 뿐, 명령은 클로드가 대신)

#### ⚙️ 먼저 알고 가야 할 3가지 개념 (1분 정리)

| 용어 | 비유 | 한 줄 풀이 |
|---|---|---|
| **Git** | 문서의 "자동 저장 + 버전 기록" | 내 PC 에서 파일 변화를 시간순으로 추적하는 프로그램 |
| **GitHub** | "원격 구글 드라이브" | Git 으로 추적한 내용을 인터넷에 백업·공유하는 사이트 |
| **저장소(Repository)** | "프로젝트 전용 폴더" | 내 앱 하나가 하나의 저장소. `my-remember-app` 이 이름 |

> 🎓 **비개발자 핵심**: 로컬 폴더(내 PC) 에서 작업 → `git commit` 으로 "세이브 포인트" 찍기 → `git push` 로 GitHub 에 업로드. 3단계 반복이 전부.

#### 🔒 0단계: 비밀키 보호 — 가장 먼저 체크!

코드를 올리기 **전에** 반드시 `.gitignore` 파일이 제대로 되어있는지 확인. 이 파일에 적힌 건 **GitHub 에 절대 안 올라가요**.

프로젝트 루트의 `.gitignore` 에 다음 3줄이 있는지 **눈으로 확인**:
...
없으면 클로드에게: **"프로젝트 루트의 `.gitignore` 에 .env.local 과 .vercel 폴더 제외하도록 추가해줘"**

> 🚨 **제가 실수한 적**: 없어요. 다행히 `create-next-app` 이 기본 `.gitignore` 를 잘 만들어 줍니다. 그래도 본인 눈으로 한 번 확인하세요. **한 번이라도 커밋되면 GitHub 이력에 영원히 남아서 삭제가 어렵습니다**.

#### ✅ 1단계: 인증 확인 (7-0 끝났으면 OK)

터미널에서 클로드에게: **"`gh auth status` 실행해서 로그인 상태 확인해줘"**

출력이 `Logged in to github.com as [내이름]` 이면 통과.

#### 🏗️ 2단계: 내 폴더를 "Git 저장소" 로 만들기 — `git init`

VS Code 터미널에서 클로드에게:

> **"현재 폴더를 git 저장소로 초기화해줘."**
...
`.git` 이라는 숨김 폴더가 생깁니다. 이제부터 이 폴더에서 생기는 파일 변화를 Git 이 추적해요.

#### 📸 3단계: 현재 상태 "세이브 포인트" 찍기 — `git commit`

지금까지 클로드가 만들어준 코드 전체를 **첫 저장점(= 커밋)** 으로 기록. 클로드에게:

> **"지금까지 만든 파일들 다 스테이징하고 '초기 프로젝트 세팅' 이라는 메시지로 첫 커밋 만들어줘. 단, `.env.local` 같은 비밀 파일은 `.gitignore` 로 제외되어야 해."**

클로드가 실행:
```bash
git status          # "어떤 파일이 추적 대상/미추적/무시되는지" 확인
git add src/ public/ package.json [...]  # 선택적 추가
git commit -m "초기 프로젝트 세팅"
```

> ⚠️ **`git add .` (점 하나) 대신 선택적 add 를 권장**
> `.` 은 "현재 폴더 전부" 란 뜻. 실수로 비밀 파일도 같이 올릴 위험.
> `.gitignore` 가 잘 작동하면 괜찮지만, **안전 2중 확인용**으로 클로드에게 "특정 파일들만 골라서 add 해줘" 가 낫습니다.

#### 🏷️ 4단계: GitHub 에 빈 저장소 만들기

클로드에게:

> **"GitHub 에 `my-remember-app` 이라는 이름으로 **Private** 저장소 만들어줘. 나중에 공개할 수도 있으니 일단 Private 으로."**

클로드가 실행:
```bash
gh repo create my-remember-app --private --source=. --remote=origin
```

위 한 줄이 하는 일:
- GitHub 에 `dctland-cloud/my-remember-app` 이라는 **빈 공간** 생성
- 내 로컬 폴더를 그 빈 공간과 **연결**(`origin` 이라는 별명으로)

> 💡 **Public vs Private — 어떤 걸 고를까?**
> - **처음에는 Private 로**: 실수로 비밀키 올라가도 나만 보이니까 안전 마진
> - 코드·매뉴얼 이상 없으면 **나중에 Public 전환** (섹션 11-6 참고)

#### 🚀 5단계: 첫 업로드 — `git push`

클로드에게:

> **"origin 에 master 브랜치로 push 해줘."**
...
`-u` 옵션은 "앞으로 `git push` 만 쳐도 이 저장소의 master 로 가라" 는 기본값 설정. 한 번만 하면 됩니다.

1~2초면 업로드 완료.

#### 🔍 6단계: GitHub 웹에서 확인

브라우저에서 👉 `https://github.com/[내계정]/my-remember-app` 접속.

**확인 체크리스트**:
- [ ] `src/`, `public/`, `package.json` 등 내 파일이 **목록에 보인다**
- [ ] 파일 목록에 `.env.local` 이 **안 보인다** (가장 중요!)
- [ ] `.gitignore` 파일은 보여도 OK (내용이 뭘 제외할지 규칙이라 공개해도 안전)
- [ ] 오른쪽 위에 "Private" 자물쇠 아이콘이 있다

> 🚨 **만약 `.env.local` 이 보인다면 즉시 조치**:
> 1. 클로드에게: **"`.env.local` 이 GitHub 에 올라가버렸어. 이력에서 완전히 지워줘."**
> 2. Firebase 콘솔·Gemini API Studio 에서 **노출된 키 즉시 재발급**
> 3. `.gitignore` 수정 후 재커밋

#### 🔄 7단계: 앞으로 코드 수정할 때마다 반복할 루틴

이제 뭔가 수정할 때마다:

```
수정 → 저장 → 클로드에게 "커밋하고 푸시해줘" → 끝
```
...
git status
git add [변경된 파일]
git commit -m "[어떤 변경인지 한 줄]"
git push
```

Vercel 이 연결되어 있으면 (7-2 섹션 참고) **푸시 직후 자동 재배포**까지 이어집니다.

#### 📝 8단계 (선택): 내 저장소 소개 문구 추가

GitHub 저장소 첫 페이지에 `README.md` 파일이 있으면 **설명문으로 노출**됩니다. `create-next-app` 이 기본 README 만들어주는데, 교재용·공유용이면 클로드에게:

> **"README.md 를 '나만의 리멤버 앱 — AI 명함 스캐너' 로 시작하는 한국어 소개문으로 다시 써줘. 주요 기능·설치법·사용법 포함."**

완료 후 `git commit` + `git push` 하면 GitHub 웹 첫 화면에 예쁘게 노출.

---

> ✅ **이 섹션 완료 조건**
> - [ ] `gh auth status` 에서 내 GitHub 계정이 로그인되어 있다
> - [ ] 내 폴더에 `.git` 숨김 폴더가 생겼다 (`git init` 완료)
> - [ ] 첫 커밋이 만들어졌다 (`git log` 로 확인 가능)
> - [ ] GitHub 웹에서 내 저장소 페이지가 열리고 파일 목록이 보인다
> - [ ] **`.env.local` 이 GitHub 에 없다** (필수!)
> - [ ] Private 저장소로 만들어졌다 (추후 Public 전환 가능)
...
> 🔧 **설치 방법 (Claude Desktop 앱의 Code 탭 사용 시)**
> 1. **Claude Desktop 앱** 이 설치돼 있어야 함 → https://claude.ai/download
> 2. Claude Desktop → Settings → **Connectors** 탭 → "Chrome" 검색 → Install → 크롬 로그인 허용
> 3. Desktop 앱 왼쪽의 **"Code" 탭** 으로 이동 → 여기서 "내 크롬 열어서 ..." 요청하면 **Connector + Code 기능이 동시에 작동**
>
> ⚠️ **주의**: 이건 **Claude Desktop 환경 전용**. 저처럼 **VS Code/Cursor/Antigravity 안의 Claude Code** 를 쓴다면 Connector 가 **직접 붙지 않습니다**. 그 환경에서는:
> - **`playwright-cli` 스킬** — 새 브라우저 창 띄워 자동화 (최초 1회 로그인 필요, 에러 13 참고)
> - **MCP 수동 설정** — `claude mcp add ...` 로 별도 서버 연결 (셋업 복잡)
>
> 💡 **비개발자에게 쉬운 순서**: Claude Desktop 의 Code 탭 → (더 깊이 가면) VS Code + playwright-cli 스킬
...
### ❌ 에러 13. Playwright 로 Google 로그인 하면 차단됨 🤖🚫 (비개발자 자동화의 최대 장벽)

**증상 (제가 실제로 겪은 것):**
- `playwright-cli open` 으로 새 브라우저 창 띄움
- Google 로그인 시도 → **"This browser or app may not be secure"** 또는 **"Couldn't sign you in"** 경고로 차단
- 내가 직접 이메일·비밀번호를 정확히 입력해도 안 됨
- Google 이 **"이 브라우저는 자동화 툴로 조종된다"** 고 감지해서 거부

**원인 (기술적):**
- Playwright 가 띄운 브라우저는 `navigator.webdriver = true` 등 자동화 지표가 붙어있음
- Google 은 이걸 탐지해서 로그인 승인 안 함 (본인이 직접 입력해도 마찬가지)

**해결 3가지 (난이도 순)**

#### 🟢 방법 1: Persistent Context — 한 번만 수동 로그인, 이후 영구 유지 (추천)

Playwright 에게 **전용 프로필 폴더**를 지정하면, 그 폴더에 쿠키·세션이 저장됩니다. 한 번만 수동 로그인해두면 다음부터는 이미 로그인된 상태로 시작돼요.

**사용법**:
```bash
# 첫 실행 — 주인님이 이 창에서 한 번 Google 로그인
playwright-cli -s=main open --persistent https://github.com

# 이후 실행 — 이미 로그인된 상태로 자동화
playwright-cli -s=main goto https://github.com/[본인]/[저장소]/settings
playwright-cli -s=main snapshot
```

**왜 이게 통하냐**: **로그인 행위 자체를 안 하니까** Google 이 탐지할 "자동화 로그인" 이 없음. 진짜로 사람이 한 번 로그인한 뒤 기계가 그 세션을 이어받는 구조.

> 💡 **클로드한테 이렇게 말하면 끝**: "playwright 로 persistent 프로필 세션 만들어서 GitHub 창 띄워줘. 내가 거기서 Google 로그인 한 번만 하면 되지?"

#### 🟡 방법 2: 이미 실행 중인 Chrome 에 CDP 연결

이미 Google 로그인된 **주인님 본인 Chrome** 에 Playwright 가 "외부에서 조작 가능한 문"을 열고 붙는 방식.

```bash
# 1. 주인님이 모든 Chrome 창 종료
# 2. 이 명령으로 Chrome 재시작 (작업표시줄 검색창에서 입력)
chrome.exe --remote-debugging-port=9222 --user-data-dir="C:\temp\chrome-debug"
# 3. 그 창에서 평소처럼 Google 로그인
# 4. Claude 에게: "CDP 로 내 크롬에 붙어서 GitHub Settings 페이지 작업해줘"
```

**단점**: 평소 쓰는 Chrome 프로필과 분리된 `user-data-dir` 을 쓰는 게 안전 (기본 프로필 쓰면 충돌). 비개발자에겐 셋업이 번거로움.

#### 🟢 방법 3: 브라우저 자동화 자체를 회피 — 공식 API 우선

GitHub·Firebase·Vercel 같은 **서비스는 전부 공식 API** 가 있습니다. OAuth 우회할 필요 없이 **토큰 발급 → curl 한 줄** 로 끝.

**제가 실제 이번에 배운 것**: GitHub 저장소 Private → Public 전환을 Playwright 로 하려다 위 장벽에 막혔고, 주인님이 **GitHub 웹에서 직접 3번 클릭** 으로 30초 만에 해결. 대규모로 자동화가 필요하면:

```bash
curl -X PATCH -H "Authorization: Bearer github_pat_..." \
...
**교훈 박스**:
> 💎 **자동화의 황금률**
> - 🟢 **로그인은 사람이 한 번만** (Persistent Context 로 쿠키 저장)
> - 🟢 **그다음은 기계가 이어받기** (같은 프로필로 자동화 재개)
> - 🟢 **서비스에 공식 API 가 있으면 무조건 그걸 먼저** (OAuth 우회 시도 금지)
> - 🔴 **매번 새 브라우저로 로그인 자동화**는 Google·Microsoft·GitHub 에서 거의 안 됨

> ⏳ **유효기간**: 이 해법은 2026년 1월 기준. Google 이 탐지 알고리즘을 업데이트하면 언젠가 방법 1·2 도 막힐 수 있습니다. 그때는 공식 API 방식(방법 3)이 가장 안전한 선택.
...
### 11-6. 코드를 GitHub 에 공개하고 싶을 때 (교재·강의용) 📢

제가 이 매뉴얼을 강의 교재로 쓰려고 GitHub 저장소를 Public(공개) 으로 바꿔봤는데, 그 과정 자체가 **비개발자에게 공부감** 이었어요. 실전 순서:

**① 비밀 파일이 올라가 있는지 확인**
```bash
# 클로드에게: "현재 git 에 .env.local 이나 API 키가 커밋돼있는지 확인해줘"
git log --all --oneline -- .env.local .env 2>&1 | head
# 결과가 비어있으면 → 한 번도 커밋 안 됨 → 안전
```

**② `.gitignore` 에 민감·불필요 파일 추가**
```
# 비밀키
.env
.env*.local
...
**③ 필요한 파일만 선택적 add** (`git add .` 금지, 특정 파일 지정)
```bash
git add docs/ .gitignore
git commit -m "docs: 강의용 매뉴얼 추가"
git push origin master
```

**④ GitHub 웹에서 Private → Public 전환** (30초)
- `https://github.com/[본인]/[저장소]/settings`
- 맨 아래 **Danger Zone** → **Change visibility** → **Make public**
- 저장소 이름 확인 입력 → 보라색 버튼

**⑤ (선택) API 로 전환** — PAT 발급이 번거로우면 ④번 웹 클릭이 훨씬 빠릅니다. 저도 결국 웹에서 3번 클릭으로 끝냈어요.

### 11-7. 두 PC 에서 작업하다 갈라졌을 때 (merge 공포증 극복) 🔀

저는 집 PC 와 노트북에서 번갈아 작업하다가 **GitHub 에 있는 커밋이 로컬에 없는 상황**을 마주쳤어요. `git push` 거부당하면:
...
# ③ 충돌 없으면 자동 커밋. 있으면 충돌 파일 수정 후:
# git add [수정된파일] && git commit

# ④ 푸시
git push origin master
```

**핵심**: `--force` 푸시는 **절대 금지**. 다른 PC 에서 한 작업이 전부 사라집니다. `merge` 가 안전 ✅.

> 🎓 **실제 제가 겪은 사례**: 노트북에서 만든 "다중 업로드 + vCard 저장" 기능이 GitHub 에만 있고 집 PC 에는 없었어요. `git merge` 한 번에 21개 파일 자동 병합되어 해결. 충돌 0건.
...
**📝 문서 버전**: v2.2 (2026-04-21 업데이트 — 섹션 7-1 "처음 GitHub 에 코드 올리기 8단계" 대폭 확장. 완전 초보자 관점에서 Git/GitHub/저장소 개념부터 첫 push 까지)
